...
export default async function AnalyticsPage({ params }: PageProps) {
  ...
  const { data: brand } = await supabase
    .from("brands")
    .select("id, name, slug, verification_tier, logo_url, total_logs, follower_count, avg_shipping")
    .eq("slug", slug)
    .eq("owner_id", user.id)
    .single();
  ...
  const isPro = brand.verification_tier === "verified" || brand.verification_tier === "partner";
  ...
  let topSlimes: { name: string; total_logs: number; avg_overall: number | null }[] = [];
  let dropPerformance: { name: string; log_count: number }[] = [];

  if (isPro) {
    const [{ data: wl }, { data: dd }, { data: ts }, { data: dp }] = await Promise.all([
      ...
      supabase.from("brand_top_slimes").select("name, total_logs, avg_overall").eq("brand_id", brand.id).order("total_logs", { ascending: false }).limit(5),
      supabase.from("drops").select("name").eq("brand_id", brand.id).order("drop_at", { ascending: false, nullsFirst: false }).limit(6),
    ]);
    ...
    topSlimes = (ts ?? []).map((s) => ({ name: s.name, total_logs: s.total_logs ?? 0, avg_overall: s.avg_overall }));
  ...
  return (
    <DashboardLayout brand={layoutBrand} active="analytics">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white" style={{ fontFamily: "Montserrat, sans-serif" }}>Analytics</h1>
        <p className="text-sm mt-1" style={{ color: "rgba(245,245,245,0.4)", fontFamily: "Inter, sans-serif" }}>Deep performance insights for your brand</p>
      </div>
      <ProGate isPro={isPro}>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-2xl p-6" style={{ background: "rgba(45,10,78,0.25)", border: "1px solid rgba(45,10,78,0.7)" }}>
              <p className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: "#00F0FF" }}>Community Ratings Breakdown</p>
          ...
          <div className="rounded-2xl p-6" style={{ background: "rgba(45,10,78,0.25)", border: "1px solid rgba(45,10,78,0.7)" }}>
            <p className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: "#00F0FF" }}>Drop Performance</p>
            <DropPerformanceChart data={dropPerformance} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {["Rating Trend Over Time", "Follower Growth"].map((label) => (
              <div key={label} className="rounded-2xl p-6 flex flex-col items-center justify-center min-h-[200px]" style={{ background: "rgba(45,10,78,0.25)", border: "1px solid rgba(45,10,78,0.7)" }}>
                <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#00F0FF" }}>{label}</p>
                <p className="text-sm" style={{ color: "rgba(245,245,245,0.35)" }}>Coming soon</p>
              </div>
            ))}
          </div>
          <div className="rounded-2xl p-6" style={{ background: "rgba(45,10,78,0.25)", border: "1px solid rgba(45,10,78,0.7)" }}>
            <p className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: "#00F0FF" }}>Export Data</p>
            <div className="flex flex-wrap gap-3">
              <button className="px-4 py-2 rounded-full text-sm font-bold" style={{ background: "linear-gradient(135deg, #39FF14, #00F0FF)", color: "#0A0A0A" }}>Export Slime Ratings CSV</button>
              <button className="px-4 py-2 rounded-full text-sm font-bold" style={{ background: "rgba(45,10,78,0.4)", border: "1px solid rgba(45,10,78,0.8)", color: "rgba(245,245,245,0.7)" }}>Export Community Logs CSV</button>
            </div>
          </div>
        </div>
      </ProGate>
