...
export default async function ProfilePage() {
  ...
  const [
    ...
    followingCountResult,
      ...
      supabase
        .from("profiles")
        .select(
          // [Change 4b] Added background_url to select
          "username, avatar_url, featured_log_ids, instagram_handle, tiktok_handle, shop_url, background_url",
        )
        .eq("id", user.id)
        .maybeSingle(),
      supabase
      ...
      supabase
        ...
        .order("created_at", { ascending: false }),
      // [T37 2026-07-13] Social stats — follower + following counts
      // pulled from `public.follows` (RLS lets anon read). Head=true
      // + count=exact means we only get the number back, no rows.
      supabase
        .from("follows")
  ...
  // [Change 4b] background_url
  const backgroundUrl = profileResult.data?.background_url ?? null;
  ...
  return (
    <PageWrapper dots glow="cyan">
      ...
      <div className="pt-14">
        {/* [Change 4b] Profile hero — outer relative wrapper for background image */}
        <div className="relative">
          {/* Background image + overlay */}
          {backgroundUrl && (
            <>
              <div className="absolute inset-0 overflow-hidden rounded-2xl mx-4 mt-2">
              ...
              <div
                className="absolute inset-0 mx-4 mt-2 rounded-2xl"
                style={{
                  background:
          ...
          <div className="relative px-4 pt-8 pb-6 overflow-hidden">
              ...
              <div className="flex-1 min-w-0">
                ...
                {isAutoGenerated ? (
                  ...
                  <div className="flex flex-col gap-0.5 mt-1">
                    <Link
                      href="/settings/profile"
                      className="inline-flex items-center gap-1 text-xs text-slime-muted font-medium active:opacity-70 transition-opacity hover:text-slime-accent"
                    >
                      <svg
                        width="14"
                        height="14"
                    ...
                    {/* [Change 4c] View Public Profile link — only when username is real */}
                    <Link
                      href={`/users/${rawUsername}`}
                      className="inline-flex items-center gap-1 text-xs text-slime-muted font-medium active:opacity-70 transition-opacity hover:text-slime-accent"
                    >
                      <svg
                        width="14"
            ...
            {/* [Change 4a] Social links — always-on brand colors */}
            {hasSocialLinks && (
              <div className="relative z-10 flex items-center gap-2 mt-3">
                {instagramHandle && (
                  <a
                    ...
                    className="flex items-center justify-center w-9 h-9 rounded-full transition-opacity active:opacity-70"
                    style={{
                      background: "rgba(45,10,78,0.25)",
                      border: "1px solid rgba(45,10,78,0.7)",
                      // [Change 4a] Instagram brand color — always on
                      color: "#E1306C",
                    }}
                  >
                    <svg
                ...
                {tiktokHandle && (
                  <a
                    ...
                    style={{
                      background: "rgba(45,10,78,0.25)",
                      border: "1px solid rgba(45,10,78,0.7)",
                      // [Change 4a] TikTok brand color — always on
                      color: "#ffffff",
                    }}
                  >
                    <svg
                ...
                {shopUrl && (
                  <a
                    ...
                    style={{
                      background: "rgba(45,10,78,0.25)",
                      border: "1px solid rgba(45,10,78,0.7)",
                      // [Change 4a] Shop brand color — always on
                      color: "#39FF14",
                    }}
                  >
                    <svg
        ...
        {/* Social stats — followers + following.
            [T37 2026-07-13] Both counts link to the public
            `/users/<username>/followers` and `/following` pages so
            the user can drill into WHO is in each list. Likes /
            comments counts are deferred until we ship the log_likes
            + log_comments schemas — see tracker. */}
        <section className="px-4 mb-6">
          ...
          <div className="flex gap-3">
            <Link
              href={`/users/${rawUsername}/followers`}
              className="flex-1 rounded-2xl px-4 py-3 transition-transform active:scale-[0.98]"
              style={{
            ...
            <Link
              href={`/users/${rawUsername}/following`}
              className="flex-1 rounded-2xl px-4 py-3 transition-transform active:scale-[0.98]"
              style={{
                background: "rgba(45,10,78,0.28)",
                border: "1px solid rgba(120,60,180,0.42)",
