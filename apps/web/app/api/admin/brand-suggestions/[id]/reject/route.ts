// apps/web/app/api/admin/brand-suggestions/[id]/reject/route.ts
//
// T110 (2026-07-11): admin rejects a community brand suggestion.
//
// Sets status='rejected', resolved_by, resolved_at, admin_notes. Fires
// a brand_suggestion_rejected notification (no brand_id — nothing to
// link to). No brands row is created.
...
export async function POST(
  ...
  if (loadErr) {
    console.error("[brand-suggestions/reject] load failed:", loadErr);
    return NextResponse.json(
  ...
  if (updateErr) {
    console.error("[brand-suggestions/reject] update failed:", updateErr);
    return NextResponse.json(
  ...
  // 3. Notify submitter (best-effort).
  if (suggestion.submitter_id) {
    ...
    if (notificationErr) {
      console.error(
        "[brand-suggestions/reject] notification insert failed:",
        notificationErr,
      );
    }
  }
