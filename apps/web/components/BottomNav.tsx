...
const NAV_ITEMS = [
  ...
  // Center — Log button (no href nav item, handled separately)
  null,
  {
    href: '/collection',
    label: 'Collection',
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-pink-100 safe-area-pb">
        <div className="flex items-end justify-around max-w-[390px] mx-auto px-2 pt-2 pb-3">
          ...
          {/* Discover */}
          <NavTab href="/discover" label="Discover" active={pathname === '/discover'}>
          ...
          >
            ...
            <span className="text-[10px] font-bold mt-1 text-pink-500">Log</span>
          </Link>

          {/* Collection */}
          <NavTab href="/collection" label="Collection" active={pathname === '/collection'}>
            {NAV_ITEMS[3]!.icon(pathname === '/collection')}
          </NavTab>

          {/* Profile */}
          <NavTab href="/profile" label="Profile" active={pathname === '/profile'}>
...
function NavTab({
  ...
  return (
    ...
    >
      {children}
      <span className={`text-[10px] font-semibold ${active ? 'text-pink-500' : 'text-gray-400'}`}>
        {label}
      </span>
    </Link>
  );
