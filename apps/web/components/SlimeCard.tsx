...
import { Slime, CollectionLog, SLIME_TYPE_LABELS, SLIME_TYPE_COLORS } from '@/lib/types';
...
export function SlimeCard({ slime, rank, onPress, className = '' }: SlimeCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${primaryColor}55, ${slime.colors[1] ?? primaryColor}33)` }}
      >
        {rank && (
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-2 mb-1.5">
          <div className="min-w-0">
            <p className="font-bold text-gray-900 text-sm leading-tight truncate">{slime.name}</p>
            <p className="text-xs text-gray-400 truncate">{slime.brand?.name}</p>
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">{slime.total_ratings.toLocaleString()} logs</span>
        </div>
      </div>
    </button>
  );
...
export function LogCard({
  ...
  const brandName = slime?.brand?.name ?? log.brand?.name ?? log.brand_name_raw ?? 'Unknown Brand';
  const primaryColor = slime?.colors?.[0] ?? '#F9A8D4';

  return (
    <article className={`bg-white rounded-2xl overflow-hidden shadow-sm border border-pink-50 ${className}`}>
      {/* User header */}
      {showUser && log.user && (
        <div className="flex items-center gap-2.5 px-4 pt-3.5 pb-2">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-800 leading-none">@{log.user.username}</p>
      ...
      {/* Slime photo placeholder */}
      <div
        className="w-full h-48 flex items-center justify-center"
        style={{ background: `linear-gradient(160deg, ${primaryColor}44, ${slime?.colors?.[1] ?? primaryColor}22)` }}
      >
      ...
      <div className="px-4 pt-3 pb-1">
        <div className="flex items-start justify-between gap-2 mb-1">
          <div className="min-w-0">
            <h3 className="font-bold text-gray-900 leading-tight">{slimeName}</h3>
          ...
          {log.rating_overall && (
            <div className="flex items-center gap-1 bg-pink-50 rounded-xl px-2.5 py-1 shrink-0">
              ...
              <span className="text-sm font-black text-pink-600">{log.rating_overall}/5</span>
            </div>
          )}
        </div>
        ...
        {log.notes && (
          <p className="text-sm text-gray-600 mt-2 leading-relaxed line-clamp-3">{log.notes}</p>
        )}
      </div>
