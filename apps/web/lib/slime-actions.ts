...
// LogSlimeInput lives in a non-"use server" module so this file can
// export only async functions (see lib/slime-actions-types.ts + the
// error-tracker "use server export trap" entry). Imported back for the
// action signatures — `import type` is erased, never re-exported.
...
// functions. Callers infer this shape from logSlime's return type.
type LogSlimeResult =
  | { ok: true; id: string }
...
// Local (not exported): "use server" files may only export async
// functions. Callers infer this shape from updateSlimeLog's return type.
type UpdateSlimeLogResult =
  | { ok: true }
  | { ok: false; error: string };
