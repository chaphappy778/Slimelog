...
//
// T127 (2026-07-21): shared reaction vocabulary + aggregation.
//
// Pure module (no "use server" / "use client", no server-only imports)
// so it can be pulled into the server actions, the feed enrichment,
// the SSR detail page, AND the client ReactionRow without dragging any
// of those runtimes into each other.
//
// This module is the SOURCE OF TRUTH for the reaction set. The DB CHECK
// constraint in 20260721000084_log_reactions.sql mirrors REACTION_TYPES
// — if the set ever changes, update BOTH (additive only: reactions
// already stored must keep validating).

// Display order matches Jennifer's T127 spec: 👍 ❤️ 🔥 🎯 🙌.
