...
| Growth / virality | 88% | — | No change today (see 2026-07-18 row) | Share-to-social deep link (T40), video reviews (T41), Momo-tier outreach (T160) |
...
| Performance + cost hardening | 55% | — | No change today (see 2026-07-18 row). The care dedupe index is additive and bounded, no new unbounded aggregates. Feed pagination shipped (T177 DONE 2026-07-17): cursor-based Load More via `/api/feed`, initial page dropped from 100 to 50 | Btree indexes on axis-sort columns (T32f), materialized views for popular aggregates, load test |
