...
## Current honest state (2026-07-18)
...
| Monetization | 55% | +25 | Prior list + AdMob account fully approved (iOS app registered + Feed Native ad unit created + app-ads.txt live at slimelog.com), ASC subscription products **both created** at correct anchor prices ($4.99 monthly + $29.99 annual base, levels set Annual=1 Monthly=2, group localization set), App Privacy nutrition label published, Stripe prices updated end-to-end (new $4.99 monthly price grandfathering old $2.99, $29.99 annual, `SLIMELOG_PRO_MONTHLY_INTRO` + `SLIMELOG_PRO_ANNUAL_INTRO` coupons for Jenn's anchor-pricing strategy), T180 collapsed since we're already at $4.99/$29.99 as base | ASC Introductory Offer sections need verification on each product, Stripe checkout code needs price ID + coupon wiring (blocked on user sending new `price_...` IDs), RC iOS SDK not wired, subscription terms display (item #14), `/settings/subscription` copy still overpromises (T171), Marketing URL in ASC v1.0 needs `https://slimelog.com` for AdMob crawler trail |
...
| Search | 50% | +35 | Item #28 Phase A shipped 2026-07-18 — `/search` now returns Brand + Collector sections alongside Slime/Type/Keyword. Empty-state copy no longer promises brands/collectors it can't deliver. `BrandRow` + `UserRow` components match SlimeRow silhouette so mixed results align. Brand query hits `brands` (name/slug/bio ilike), User query hits `profiles_public` view. Type-check clean | Phase B: relevance ranking (exact-match → prefix → substring), graceful degradation. Phase C (post-launch): typeahead dropdown, faceted filters. T161 (personal /collection search) still separate |
...
1. **Search: 50 → 70 (+20).** MASSIVELY reduced from +55 with Item #28 Phase A landing. Brand + Collector sections now render. Phase B (relevance ranking) closes the remaining gap. Global search is table-stakes when users hear about slimes on IG and land wanting to look them up — Phase A means someone searching "aloe nightmares" or "@jenn_slimelogapp" now gets results instead of a zero-state.
...
13. **Monetization: 55 → 30 (0).** WELL PAST target. Now the biggest coiled spring for iOS App Store phase: real closure to 90% only comes when RC SDK ships in Capacitor + FeedAdSlot component renders live ads, but the entire external chain (AdMob approval + ASC subscription products + Stripe intro-price coupons + Play Console account) is now cleared.
...
- **Monetization should not fall more than 40 points behind consumer visual.** We're at 55 vs 84 (29-point gap, down from 54). Big win from today's setup marathon — AdMob approved + ASC subscription products live + Stripe intro pricing wired. RC SDK + FeedAdSlot component are the two remaining pieces for full closure at iOS App Store phase.
