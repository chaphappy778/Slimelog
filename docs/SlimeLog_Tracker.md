...
| T113 | Marketplace Coming Soon page + waitlist | HIGH | READY | 2026-07-12. `/marketplace` page — hero announcement + two-intent (seller / buyer / both) waitlist + optional research questions (brand interest, price band, monthly volume, trust-need open text) + confirmation state with real waitlist position and share CTA. Design brief handed to Claude Design 2026-07-12. Backend: `marketplace_waitlist` table with intent enum, optional research fields, submitter_id, `?ref=CODE` referral wire-up. Ships as: (a) design mockup review, (b) migration, (c) POST /api/marketplace/waitlist, (d) /marketplace page + components. Marketplace itself is a 6-12 month V2; this page is the announcement + audience-building surface. See docs/monetization-plan-2026-07-07.md for full marketplace strategy. |
| T114 | Slime condition field on logs | LOW | DONE | 2026-07-12. Added `condition` enum column to collection_logs (sealed / new / like_new / used / well_loved). Migration 20260712000067. Wired into log wizard as an optional pill picker beside scent strength; each pill has a hover title with a plain-English description. Serves two purposes: personal shelf tracking (users can note dried-out or sealed slimes) and future marketplace listing schema (T113 → full marketplace). |
