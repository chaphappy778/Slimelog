...
| T104 | Client-side auth.getUser duplication | MEDIUM | DONE | 2026-07-10: shipped `<AuthProvider>` context in components/AuthProvider.tsx that fetches user + profile once on mount and syncs via onAuthStateChange. Refactored 5 clients that were each firing their own auth.getUser + profile query: ShareButton, OnboardingGate, OnboardingModal, SlimeMenu, ClientComments. Profile payload now selects all needed columns (username, avatar_url, display_name, role, referral_code, onboarding_completed_at, subscription_tier, is_premium) once, so future consumers just add the column here rather than fanning out new queries. |
