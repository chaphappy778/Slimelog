...
| T32f | Wire /discover?sort=&lt;axis&gt; deep-links | LOW | READY | 2026-07-13. /how-to-rate axis sections now include a `See top-rated slimes on {axis} →` link that resolves to `/discover?sort={slug}` for `texture / sound / aesthetic / creativity / quality / overall`. Discover doesn't currently accept those query params — links will 404 or fall through to the default view. Add query-param support so a click on any axis link scrolls the user straight into a filtered discover surface sorted by that dimension. Slug → column mapping: texture → rating_texture, sound → rating_sound, aesthetic → rating_drizzle (DB column name), creativity → rating_creativity, quality → rating_sensory_fit (DB column name), overall → rating_overall. |
