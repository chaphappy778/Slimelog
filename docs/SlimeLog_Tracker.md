...
| T32e | Redesign /how-to-rate to match guide layout | MEDIUM | READY | 2026-07-13. The current 776-line /how-to-rate page uses the same gradient card pattern as the OLD /slime-types page (pre-guide-redesign). Once we ship /guide and confirm it lands well, bring the same treatment (sticky pill nav, per-axis detail sheet, prose sections, tone-tinted callouts) to /how-to-rate so the two educational surfaces feel like siblings. Design brief handed off 2026-07-13. **Bundle in with this batch:** color-code the glossary tags across the guide so they align with /how-to-rate's dimension colors. Currently Part 2 Add-ins (crunch/deco/body/family), Part 4 Scents (family/descriptor), Part 10 Aesthetic (palette/theme), and Part 11 Sound (sound) all render pills in the SAME accent color per section — no visual differentiator between "crunch" and "foundation" or between "palette" and "theme". `GlossaryList` should accept a `tagColorMap` (or per-entry color) so tags inherit the same color language as their matching rating axis on /how-to-rate. |
