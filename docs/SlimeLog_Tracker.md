...
| T111 | Content moderation for user-generated names + submissions | MEDIUM | BACKLOG | 2026-07-11. We currently have no restrictions on usernames, slime names, brand suggestions, or brand-name-raw fields. Before we open user-authored content wider we need: (a) canonical banned-words list, (b) reserved-username list (admin-only, "official", "slimelog", etc.), (c) shared validation helper `lib/moderation.ts` that returns `{ ok } | { ok: false, reason }` — used by all user-submitted text (username set, brand suggestion name/note, slime name, collection name, etc.), (d) profanity library or fuzzy match for common obfuscations, (e) apply retroactively via nightly job for existing fields. Ship BEFORE the brand-suggestion flow gets heavy usage. Follow-up to T110. |
