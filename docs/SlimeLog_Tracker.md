...
| T104 | Client-side auth.getUser duplication | MEDIUM | READY | 2026-07-10, filed during T101 profiling. DevTools on /slimes/[id] showed `user` fetched 3× per page load — multiple client components each call supabase.auth.getUser() independently (ShareButton, LikeButton, ReportButton, likely others). Fix by hoisting auth state to a client-side context / hook that fans in via a single getUser call and shares the result across components. Same pattern also produces 3 separate profiles queries (referral, role, username) that could consolidate. |
